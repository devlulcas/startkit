import { hueToDegree, rgbaLimitedAlpha, rgbFromHslValues } from "./ColorMath";

type ColorTuple = [number, number, number, number];
type HexaTuple = [string, string, string, string];

function transformUnknown(color: string) {
	...
	if (firstLetter === "r") {
		rgbaValue = rgbaToRgbaArray(treatedInput);
		hslaValue = rgbaToHsla(...rgbaValue);
		hexaValue = rgbaToHexa(...rgbaValue);
	} else if (firstLetter === "h") {
		rgbaValue = hslaToRgbaArray(treatedInput);
		hslaValue = rgbaToHsla(...rgbaValue);
		hexaValue = rgbaToHexa(...rgbaValue);
	} else {
		rgbaValue = hexaToRgbaArray(treatedInput);
		hslaValue = rgbaToHsla(...rgbaValue);
		hexaValue = rgbaToHexa(...rgbaValue);
	}
...
function rgbaToRgbaArray(rgba: string): ColorTuple {
...
function hexaToRgbaArray(hexa: string): ColorTuple {
	const replaceable = /["#"]/g;
	...
	const alpha = rgbaLimitedAlpha(a);

	return [r, g, b, alpha];
}

// Receives a hsla string, returns an array of red, green, blue and alpha
function hslaToRgbaArray(hsla: string): ColorTuple {
	...
	const hueInDegrees = hueToDegree(h);
	...
	const rgb = rgbFromHslValues(hueInDegrees, chroma, secondLargestComponent, matchLightness);
...
function rgbaToHsla(r: number, g: number, b: number, a: number): ColorTuple {
...
function rgbaToHexa(r: number, g: number, b: number, a: number): HexaTuple {
